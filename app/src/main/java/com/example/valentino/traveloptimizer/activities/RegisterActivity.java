...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void init() {
        ...
        address.addTextChangedListener(new TextWatcher() {
            ...
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {
                if (s.length() == 0) {
                    TextView label = (TextView) findViewById(R.id.textViewRegAddressLabel);
                    label.setText(address.getHint() + " with country code and no seperators");
                    address.setBackground(getDrawable(R.drawable.text_border_selector));
                }
            }

            @Override
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                TextView label = (TextView) findViewById(R.id.textViewRegAddressMessage);
                label.setText("");
            }
